<script lang="ts">
  ...
  import CommentWriter from "./CommentWriter.svelte";
...
<div class={`p-3 rounded-lg ` + addition}>
  <div class="flex justify-between">
    ...
    <button on:click={()=>{ openCom = !openCom}} type="button" class="cursor-pointer underline">답글</button>
  </div>
  <p class="text-sm">{data.comment}</p>
</div>
<CommentWriter postId={data.postId} parent={data.id} openComment={openCom}/>